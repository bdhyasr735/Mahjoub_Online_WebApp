...
# استخدام اسم فريد ومستقل لاسم الـ Blueprint الداخلي لتجنب التعارض أثناء التسجيل في التطبيق
suppliers_dashboard_bp = Blueprint(
    'suppliers_dashboard_core_module',
    __name__,
    url_prefix='/supplier',
...
# نسخ مرجعية للحفاظ على توافق الاستيرادات والمسارات في بقية النظام
...
def supplier_login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'supplier_id' not in session:
            ...
            return redirect(url_for('suppliers_dashboard_core_module.login'))
...
def login():
    if 'supplier_id' in session:
        return redirect(url_for('suppliers_dashboard_core_module.dashboard'))
