...
def _sync_products_in_background(supplier_id):
    ...
    """
    try:
        # جلب جميع الروابط النشطة للمورد
        ...
        for mapping in mappings:
            ...
            # جلب تفاصيل المنتج من الـ API (استعلام واحد فقط لكل منتج)
            product_data = services.products.get_product_by_qid(qid)
            if product_data:
                # تحديث وقت المزامنة (اختياري، يمكننا تحديث حقل آخر)
                mapping.updated_at = datetime.utcnow()
                updated_count += 1
            else:
                # إذا لم يوجد المنتج في الـ API، يمكننا تركه أو تحديث حالته
                pass
...
def manage_supplier_products_view():
    try:
        ...
        # ============================================================
        # المزامنة الخلفية الذكية (مرة كل 10 دقائق)
        # ============================================================
        ...
        # ============================================================
        # جلب المنتجات المرتبطة فقط مع Pagination من قاعدة البيانات
        # ============================================================
        ...
        # (اختياري) نفلتر حسب الحالة 'active' إذا أردنا
        # query = query.filter_by(status='active')
        pagination = query.paginate(page=page, per_page=per_page, error_out=False)
        mappings = pagination.items

        # ============================================================
        # جلب تفاصيل المنتجات من الـ API (لكل منتج على حدة)
        # ============================================================
        products_data = []
        for mapping in mappings:
            qid = mapping.product_qid
