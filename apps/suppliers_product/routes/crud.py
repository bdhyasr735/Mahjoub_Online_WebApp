...
def edit_supplier_product(product_qid):
    try:
        ...
        # 2. جلب المنتج مع حماية ذكية ضد أخطاء الـ QID
        product = None
        try:
            product = services.products.get_product_by_qid(product_qid)
        except Exception as q_err:
            current_app.logger.warning(f"تعذر جلب المنتج بالمعرف الخارجي {product_qid}: {q_err}")

        if not product:
            flash('⚠️ تنبيه: تعذر استعلام تفاصيل المنتج من الخادم الخارجي، يمكنك تعديل البيانات المتاحة محلياً.', 'warning')
            product = {
                'title': mapping.product_qid, 
                'price': getattr(mapping, 'price', 0), 
                'quantity': getattr(mapping, 'quantity', 0)
            }
