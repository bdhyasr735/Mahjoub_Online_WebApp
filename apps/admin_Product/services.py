...
class ProductService:
    ...
    # محاكاة تخزين البيانات مؤقتاً في الذاكرة / التعامل مع DB Session
    _products_db = [
        {
            "id": "prod_101",
        ...
        {
            "id": "prod_103",
            "title": "سماعات لاسلكية محجوب باص إكستريم",
            "slug": "mahjoub-bass-extreme-wireless-headphones",
            "status": "draft",
            "description": "سماعة رأس لاسلكية بخاصية إلغاء الضوضاء النشط ANC وصوت نقي جداً مع تقنية الصوت المحيطي.",
            "price": 599.00,
            "compareAtPrice": 799.00,
            "quantity": 30,
            "sku": "M3-HDP-EXT",
            "barcode": "6291048201888",
            "collections": ["الصوتيات", "الإلكترونيات"],
            "tags": ["سماعات", "إلغاء الضوضاء", "بلوتوث"],
            "images": [
                {"fileUrl": "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=600&auto=format&fit=crop", "isMain": True, "alt": "سماعات محجوب باص"}
            ],
            "seo": {
                "title": "سماعات لاسلكية محجوب باص إكستريم - عزل ضوضاء فاخر",
                "description": "استمتع بعزل صوتی مثالي وصوت جهير مع سماعات محجوب باص إكستريم الحصرية.",
                "canonicalUrl": "https://www.mahjoub.online/products/mahjoub-bass-extreme-wireless-headphones"
            },
            "dynamic_variants": [],
    ...
    def generate_slug(cls, title: str) -> str:
        """إنشاء رابط صديق للرابط (slug) بناء على العنوان"""
        if not title:
            return ""
        # تحويل المسافات إلى شُرط وتنظيف الرموز الخاصة
    ...
    def get_products_page(cls, page: int = 1, per_page: int = 10, search: str = "", status: str = "", collection: str = ""):
        """جلب قائمة المنتجات مع التصفية والبحث وعرض الصفحات"""
    ...
    def create_product_data(cls, data: dict):
        """إضافة منتج جديد مع كافة تفاصيل التسعير والمخزون والمتغيرات والـ SEO"""
        new_id = f"prod_{int(datetime.now().timestamp())}"
        title = data.get('title', '').strip()
        slug = data.get('slug', '').strip() or cls.generate_slug(title)

        # حساب إجمالي الكمية من المتغيرات إن وجدت
    ...
    def toggle_product_status(cls, product_id: str, new_status: str):
        """تحديث حالة المنتج (نشط / مسودة / مؤرشف)"""
        product = cls.get_product_by_id(product_id)
        if product:
