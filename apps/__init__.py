# coding: utf-8
# 📂 apps/__init__.py - المصنع السيادي (النسخة النهائية المعتمدة)
...
def create_app():
    ...
    with app.app_context():
        try:
            # استيراد النماذج فقط عند الحاجة لتجنب الحلقات (Circular Imports)
            from apps.models.admin_db import AdminUser
            ...
            if not AdminUser.query.filter_by(username='علي_محجوب').first():
                ...
                db.session.commit()
                print("✅ تم تأسيس الحساب السيادي بنجاح.")
        except Exception as e:
            db.session.rollback()
            print(f"⚠️ خطأ أثناء التأسيس السيادي: {e}")
