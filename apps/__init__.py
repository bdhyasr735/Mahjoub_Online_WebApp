...
# coding: utf-8
# 📂 apps/__init__.py - المصنع السيادي الموحد (النسخة المستقرة)
...
def create_app():
    ...
    with app.app_context():
        try:
            ...
            raise # نرفع الخطأ لنعرف الموديول المتسبب في الـ Status 1

    # التوجيه الأساسي (استخدام url_for بدلاً من المسار النصي لتفادي الأخطاء)
    @app.route('/')
    def index():
...
def create_app():
    ...
    # 4. تسجيل المسارات (Lazy Loading لتجنب فصل السيرفر)
    with app.app_context():
        from apps.auth_portal.routes import auth_portal
        from apps.admin_dashboard.routes import admin_dashboard
        from apps.wallet.routes import wallet_app
