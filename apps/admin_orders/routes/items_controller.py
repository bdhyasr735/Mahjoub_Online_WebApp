...
def _parse_datetime(dt_str):
  """تحويل سلسلة التاريخ ISO القادمة من API المنصة إلى كائن datetime آمن لحمايته من أخطاء PostgreSQL"""
  if not dt_str:
    return None
...
def _save_or_update_order_item(order_id, item_data):
  ...
  item = OrderItem.query.filter_by(
      order_id=order_id, product_qid=product_id
  ).first()
  if not item:
  ...
  if not supplier_id:
    mapping_match = ProductSupplierMapping.query.filter_by(
        product_qid=product_id
    ).first()
    if mapping_match:
...
def _save_or_update_order(order_data):
  """حفظ أو تحديث الطلب وعناصره وتنفيذ commit آمن واحد فقط مع حماية من أخطاء Rollback"""
  ...
  try:
    ...
    if isinstance(status_obj, dict):
      ...
      order.status_title = status_obj.get(
          'title', STATUS_TITLES_MAP.get(order.status_code, 'قيد الانتظار')
      )
    else:
      order.status_code = str(status_obj)
      order.status_title = STATUS_TITLES_MAP.get(
          order.status_code, 'قيد الانتظار'
      )
    ...
    order.created_at = _parse_datetime(order_data.get('createdAt'))
    updated_at = order_data.get('updatedAt')
    if updated_at:
      order.updated_at = _parse_datetime(updated_at)
    ...
    current_app.logger.error(
        f'❌ فشل حفظ الطلب {order_id} في قاعدة البيانات: {e}'
    )
...
def sync_all_orders_from_graphql(max_pages=None):
  ...
  print(
      f'🏁 [DEBUG] انتهت المزامنة، إجمالي الطلبات التي تم جلبها وحفظها:'
      f' {total_synced}'
  )
  return total_synced
  ...
  with app.app_context():
    try:
      ...
      current_app.logger.info(
          f'✅ تمت مزامنة {total} طلباً في الخلفية (أول 5 صفحات)'
      )
    except Exception as e:
  ...
  try:
    ...
    if not is_ajax:
      ...
      thread = threading.Thread(
          target=_sync_orders_from_graphql, args=(app,), daemon=True
      )
      thread.start()
    ...
    # 1. فلتر حالة الطلب
    status_filter = request.args.get('status') or request.args.get(
        'orderStatusFilter'
    )
    if status_filter:
    ...
    if payment_status is not None and payment_status != '':
      is_paid_bool = (
          True if str(payment_status).lower() in ['true', '1', 'yes'] else False
      )
    ...
    if supplier_filter and supplier_filter != '':
      if supplier_filter == 'none':
        subquery_with_supplier = (
            db.session.query(OrderItem.order_id)
            .filter(OrderItem.supplier_id != None)
            .distinct()
        )
        query = query.filter(
            db.and_(
        ...
        matching_order_ids = (
            ...
            .outerjoin(
                ...
                OrderItem.product_qid == ProductSupplierMapping.product_qid,
            )
            .filter(
                or_(
                    OrderItem.supplier_id == s_id_str,
                    *(
                        [OrderItem.supplier_id == s_id_int]
                        if s_id_int is not None
                        else []
                    ),
                    ProductSupplierMapping.supplier_id == s_id_str,
                    *(
                        [ProductSupplierMapping.supplier_id == s_id_int]
                        if s_id_int is not None
                        else []
                    ),
                )
    ...
    if is_ajax:
      return jsonify({
          ...
          'html': render_template(
              'admin/partials/_orders_table.html', orders=orders
          ),
          'pagination_html': render_template(
              'admin/partials/_pagination.html', pagination=pagination_info
          ),
      })
...
def sync_admin_orders():
  try:
    ...
    return jsonify({
        'success': True,
        'message': (
            '⏳ تم بدء المزامنة الكاملة في الخلفية. سيتم تحديث الجدول'
            ' تلقائياً...'
        ),
    })
  except Exception as e:
...
@admin_orders_bp.route(
    '/<string:order_id>/sync', methods=['POST'], endpoint='sync_single_order'
)
@login_required
def sync_single_order(order_id):
  try:
    ...
    current_app.logger.error(
        f'خطأ في مزامنة الطلب {order_id}: {traceback.format_exc()}'
    )
    return jsonify({'success': False, 'message': str(e)}), 500


@admin_orders_bp.route(
    '/<string:order_id>', methods=['GET'], endpoint='view_admin_order'
)
@login_required
def view_admin_order(order_id):
  try:
    ...
    return render_template(
        'admin/admin_order_detail.html', order=order, items_list=items
    )
  except Exception as e:
    current_app.logger.error(
        f'خطأ في عرض تفاصيل الطلب: {traceback.format_exc()}'
    )
    flash(f'❌ حدث خطأ: {str(e)}', 'danger')
    return redirect(url_for('admin_orders_bp.list_admin_orders'))
...
@admin_orders_bp.route(
    '/<string:order_id>/invoice',
    methods=['GET'],
    endpoint='print_order_invoice',
)
@login_required
def print_order_invoice(order_id):
  ...
  try:
    ...
    items = OrderItem.query.filter_by(order_id=order_id).all()
    return render_template(
        'admin/order/print_invoice.html', order=order, items_list=items
    )
  except Exception as e:
...
@admin_orders_bp.route(
    '/<string:order_id>/update-status',
    methods=['POST'],
    endpoint='update_order_status_inline',
)
@login_required
def update_order_status_inline(order_id):
  try:
    ...
    qumra_success = services.orders.update_order_status_in_qumra(
        order_id, new_status
    )

    if not qumra_success:
      return jsonify(
          {'success': False, 'message': 'فشل تحديث الحالة في المنصة (قمره)'}
      ), 500
    ...
    return jsonify({
        'success': True,
        'message': 'تم تحديث الحالة في المنصة والمزامنة بنجاح',
    })
  except Exception as e:
    current_app.logger.error(
        f'خطأ في تحديث حالة الطلب: {traceback.format_exc()}'
    )
    return jsonify({'success': False, 'message': str(e)}), 500
...
def api_update_order_number():
  ...
  try:
    ...
    if not external_order_id:
      return (
          jsonify({'success': False, 'message': 'external_order_id مطلوب'}),
          400,
      )
    ...
    current_app.logger.error(
        f'خطأ في تحديث رقم الطلب: {traceback.format_exc()}'
    )
    return jsonify({'success': False, 'message': str(e)}), 500
